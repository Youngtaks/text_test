...
<script>
	function send(){
		if(!frm.t_no.value){
			alert("입출고번호가 일치하지 않습니다.");
			frm.t_no.focus();
			return;
		}
		if(!frm.p_code.value){
			alert("제품코드가 일치하지 않습니다.");
			frm.p_code.focus();
			return;
		}
					...
					<form name="frm" method="post" action="insert_pro.jsp">
						<table>
							...
							<tr>
								<th>입출고구분</th>
								<td>
									<input type="radio" value="I" name="t_type" size=2 checked>입고
									<input type="radio" value="0" name="t_type" size=2>출고
								</td>
							</tr>
								<tr>
								<th>수량</th>
